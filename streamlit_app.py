...
lcm_answers = st.secrets["lcm"]

lcm_user_answers = {}

def parse_multiples(input_str):
    # Parse input for multiples, splitting on commas, semicolons, colons, dots, hyphens, spaces
...
for qnum, (num1, num2) in lcm_questions.items():
    st.write(f"{qnum}. Find the LCM of **{num1} and {num2}**.")
    multiples_1 = st.text_input(f"Multiples of {num1}", key=f"lcm{qnum}_num1")
    multiples_2 = st.text_input(f"Multiples of {num2}", key=f"lcm{qnum}_num2")
    lcm = st.text_input(f"LCM", key=f"lcm{qnum}_lcm")
    st.write("")  # blank line
    st.write("")  # another blank line
    lcm_user_answers[qnum] = {
        "num1": multiples_1,
        "num2": multiples_2,
        "lcm": lcm,
    }
